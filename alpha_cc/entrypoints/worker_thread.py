...
from alpha_cc.engine import Board, GameConfig, ImprovedHalvingParams, PuctFreeParams
...
@click.option(
    "--mcts-variant",
    type=click.Choice(["puct_free", "improved_halving"]),
    default="puct_free",
    help="Which MCTS flavour: PUCT descent + free parallel rollouts, or improved-policy descent + sequential halving.",
)
# improved_halving options
...
# puct_free options (defaults match master's compose.yaml)
@click.option("--c-puct-init", type=float, default=2.0, help="Initial PUCT exploration constant")
@click.option("--c-puct-base", type=float, default=10000.0, help="PUCT exploration log-scaling base")
@click.option("--dirichlet-weight", type=float, default=0.15, help="Root dirichlet noise blend weight (0 = disabled)")
@click.option("--dirichlet-alpha", type=float, default=0.15, help="Root dirichlet concentration")
@click.option("--puct-temperature", type=float, default=1.0, help="Temperature for visit-count pi in FreeScheduler")
...
def main(
    ...
    def build_mcts_params() -> tuple[ImprovedHalvingParams | None, PuctFreeParams | None]:
        if mcts_variant == "improved_halving":
            return (
        ...
        if mcts_variant == "puct_free":
            return (
                ...
                PuctFreeParams(
                    ...
                    temperature=puct_temperature,
                ),
            )
        raise ValueError(f"unknown mcts_variant={mcts_variant!r}")

    def create_model(channel: int, trainer_time: int, dummy_preds: bool) -> MCTSAgent:
        improved_halving, puct_free = build_mcts_params()
        return MCTSAgent(
