...
class StandaloneMCTSAgent(MCTSAgent):
    ...
    def choose_move(  # type: ignore
        ...
        result = self.run_rollouts(board, n_rollouts=n_rollouts, rollout_depth=rollout_depth, temperature=temperature)
    ...
    def run_rollouts(
        ...
        value = float(
            -np.array([self._rollout(state, remaining_depth=rollout_depth) for _ in range(n_rollouts)]).mean()
        )
        pi = self._rollout_policy(state.board, temperature)
        # Derive WDL from scalar value (no full WDL tracking in Python MCTS)
        mcts_wdl = ((1 + value) / 2, 0.0, (1 - value) / 2)
        return RolloutResult(pi.tolist(), value, mcts_wdl)
