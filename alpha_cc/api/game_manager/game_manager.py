...
from alpha_cc.agents import GreedyAgent
...
class GameManager:
    def __init__(self, db: DB) -> None:
        self._db = db
        self._agents = {size: GreedyAgent(size) for size in self.supported_sizes}
    ...
    def create_game(
        ...
        if starting_player is not None:
            board = board.reset_with_starting_player(starting_player)
    ...
    def apply_move(self, game_id: str, move_index: int) -> tuple[Move, Board]:
        ...
        move = db_state.state.board.get_legal_moves()[move_index]
